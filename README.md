...
国土数値情報（nlftp.mlit.go.jp）の行政区域・将来推計人口・鉄道・医療機関・学校・バス・洪水浸水想定区域の GIS データを DuckLake カタログ化したデータセット。
...
9. `bash scripts/build.sh` でビルドする。
...
国土数値情報の利用約款および政府標準利用規約（第2.0版）に従う。出典データは加工して利用している。洪水浸水想定区域データ（A31a、2021年度以降版）とバスルートデータ（N07、令和4年度版）のライセンスは CC BY 4.0。バス停留所データ（P11）は第3.0版（令和4年度）のみが国土数値情報ダウンロードサイトコンテンツ利用規約に基づくオープンデータで、第2.0版（平成22年度）は非商用のため使用していない。
