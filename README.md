...
国土数値情報（nlftp.mlit.go.jp）の行政区域・将来推計人口・鉄道・医療機関・学校・洪水浸水想定区域・用途地域の GIS データを DuckLake カタログ化したデータセット。
...
8. 用途地域データ（A29）はパイプライン実行時に都道府県単位の zip を自動ダウンロードし、オープンデータ公開可の市区町村の GeoJSON を Parquet に変換する。
9. `bash scripts/build.sh local` でビルドする。
